...
def preprocess_pnds():
    ...
    spec_dict = dict(zip(specialty_df['Code'],specialty_df['Provider Specialty/Service Description']))
    ...
    df = df[(df['provtype']==1) | (df['provtype']==12) | (df['provtype']==2) | (df['provtype']==23)].copy()
    provtype_dict = {1: 'physician', 12: 'physician', 2: 'nurse practitioner', 23: 'physician assistant' }
    ...
    df = df.rename({'nppes_type': 'provider_nppes_type', 'nppes_status':'provider_nppes_status', 
               'nppes_name':'provider_nppes_name'}, axis=1)
    ...
    df = df.rename({'nppes_type': 'facility_nppes_type', 'nppes_status':'facility_nppes_status', 
               'nppes_name':'facility_nppes_name'}, axis=1)
    ...
    df['sitenpi'] = df.apply(lambda x: remove_incorrect_npi(x['facility_nppes_type'], x['sitenpi']), axis=1)
    ...
    df_grp['licnum'] = df_grp.apply(lambda x: keep_max_frequency(x['licnum_count'], 
                                                                 x['licnum_max'], 
                                                                 x['licnum']), axis=1)
    ...
    df['latlong'] = df.apply(lambda x: swap_latlong(x['Latitude'], x['Longitude']), axis=1)
    ...
    df['name_score'] = df.apply(lambda x: fuzz.partial_ratio(x['pnds_name'],x['provider_nppes_name']),axis=1)
    ...
    df['secondary_language'] = df.apply(lambda x: extract_secondary_language(x['lang1'], x['lang2']), axis=1)
    ...
    df[['AddressNumber', 'StreetName', 'StreetNameSuffix']] = df[['AddressNumber', 'StreetName', 'StreetNameSuffix']].replace({np.nan: None})
    ...
    df['sitename'] = df.apply(lambda x: sitename_update(x['sitename'], x['StreetName'], x['StreetNameSuffix']), axis=1)
    ...
    df['sitename'] = df.apply(lambda x: sitename_update(x['sitename'], x['probablepeople']), axis=1)
    df['sitename'] = df.apply(lambda x: sitename_realname(x['sitename'], x['StdFirstName'], x['StdLastName']), axis=1)
    ...
    df = df.drop(columns=['AddressNumber', 'PlaceName', 'StateName', 'StreetName', 'StreetNamePreDirectional',
                         'StreetNamePostDirectional', 'ZipCode', 'StreetNamePrefix', 'StreetNameSuffix', 'USPSBox',
                         'OccupancySuite', 'probablepeople'])
    ...
    df['physfeinloc'] = df['npi'].astype(str) + df['fein'].astype(str) + df['latlong'].astype(str)
    ...
    df = df.drop_duplicates(subset=['npi', 'fein', 'Address1_std', 'City_std', 'Zip_std']).copy()
