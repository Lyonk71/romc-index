...
from modules.linkage.project_functions import (
    standardize_licnum,
    low_confidence
)
...
def sed_to_pnds():
    ...
    # import sed
    df_sed = pd.read_csv('input_datasets/LICENSE_NON_CONF_ACTIVE.csv', dtype=str, low_memory=False)
    df_sed = df_sed[['LICENSE_NUMBER', 'NAME', 'AGE_END_OF_YEAR']]
    df_sed = df_sed.drop_duplicates()
    df_sed = df_sed.dropna()
    
    # import pnds
    df_pnds = pd.read_csv('intermediate_datasets/clustered_addresses.csv', dtype=str, low_memory=False)
    df_pnds = df_pnds.rename({'cluster id': 'pnds_cluster_id', 'confidence': 'pnds confidence'}, axis=1)
    ...
    # link pnds & sed
    dfq = pandas_dedupe.link_dataframes(df_pnds_subset, df_sed, [('LICENSE_NUMBER', 'Exact'),
                                                                 'NAME'], config_name='dedupe_settings/sed_to_pnds')
    # separate datasets
    df_pnds_subset = dfq[dfq['dataset']=='pnds'].copy()
    df_sed = dfq[dfq['dataset']=='sed'].copy()
    
    # remove low confidence scores from each dataset
    df_pnds_subset['confidence'] = df_pnds_subset.apply(lambda x: low_confidence(x['confidence'], x['confidence']), axis=1)
    df_pnds_subset['cluster id'] = df_pnds_subset.apply(lambda x: low_confidence(x['cluster id'], x['confidence']), axis=1)
    
    df_sed['confidence'] = df_sed.apply(lambda x: low_confidence(x['confidence'], x['confidence']), axis=1)
    df_sed['cluster id'] = df_sed.apply(lambda x: low_confidence(x['cluster id'], x['confidence']), axis=1)
    ...
    df_sed = df_sed.rename({'LICENSE_NUMBER': 'licnum', 'AGE_END_OF_YEAR': 'age'}, axis=1).copy()
